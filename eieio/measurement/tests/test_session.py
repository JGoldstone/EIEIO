...
from eieio.measurement.session import Origin, Colorimetry, Session
from datetime import datetime

EIEIO_PATH = f"./{Session.EIEIO_CONFIG}"
EIEIO_DATA_DIR = '../../data'
...
def is_parseable_date(str_):
    try:
        datetime.strptime(str_, Session.TIMESTAMP_FORMAT)
    except ValueError:
        return False
    return True
...
class MyTestCase(unittest.TestCase):
    ...
    def test_check_path_is_writable_plain_file(self):
        try:
            ...
            self.assertRaises(FileExistsError, Session.check_path_is_writable_dir, EIEIO_PATH,
                              "a plain file (for which we DO have write permission")
        finally:
            Path(EIEIO_PATH).unlink(missing_ok=True)

    def test_check_path_is_writable_no_write_permission(self):
        self.assertRaises(ProcessLookupError, Session.check_path_is_writable_dir, '/etc',
                          "a directory for which we lack write permission")

    def test_default_path_from_eieio_from_component(self):
        try:
            write_test_eieio_config("/Users/share/data")
            p = Session.default_path_component_from_eieio("/foo/bar", '/tmp/default_default',
    ...
    def test_EIEIO_default_path_from_eieio_from_eieio_TL_bad(self):
        try:
            ...
            p = Session.default_path_component_from_eieio(None, '/tmp/default_default', 'x_top_level')
    ...
    def test_EIEIO_default_path_from_eieio_from_eieio_TL_good(self):
        try:
            ...
            p = Session.default_path_component_from_eieio(None, '/tmp/default_default', 'top_level')
    ...
    def test_EIEIO_default_path_from_eieio_from_eieio_bad(self):
        try:
            write_test_eieio_config(None, None)
            p = Session.default_path_component_from_eieio(None, '/tmp/default_default',
    ...
    def test_session_ctor(self):
        ...
        self.assertEqual(len(session._dirty_keys), 0)
    ...
    def test_spectral_loading(self):
        ...
        self.assertEqual(len(session._dirty_keys), 1)
        self.assertEqual(len(session._tscs), 0)
        self.assertEqual(len(session._dirty_tscs_keys), 0)
