...
def write_blank_EIEIO_config(base_dir_path=None, dir_name=None):
...
class MyTestCase(unittest.TestCase):
    ...
    def test_default_path_from_eieio_from_component(self):
        try:
            write_test_EIEIO_config("/Users/share/data")
            p = ms.MeasurementSession.default_path_component_from_eieio("/foo/bar", '/tmp/defdef', 'measurements.base_dir_path')
    ...
    def test_EIEIO_default_path_from_eieio_from_eieio_bad(self):
        try:
            write_test_EIEIO_config(None, None)
            p = ms.MeasurementSession.default_path_component_from_eieio(None, '/tmp/defdef', 'measurements.x_base_dir_path')
            self.assertEqual(p, "/tmp/defdef")
        finally:
            Path(EIEIO_PATH).unlink(missing_ok=True)
    ...
    def test_EIEIO_default_path_from_eieio_from_eieio_good(self):
        try:
            write_test_EIEIO_config("/var/tmp/data", None)
            p = ms.MeasurementSession.default_path_component_from_eieio(None, '/tmp/defdef', 'measurements.base_dir_path')
